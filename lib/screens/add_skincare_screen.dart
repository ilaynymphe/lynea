...
class AddSkincareScreen extends StatefulWidget {
  final SkincareItem? existingItem;
  final String? initialName;

  const AddSkincareScreen({super.key, this.existingItem, this.initialName});

  @override
  State<AddSkincareScreen> createState() => _AddSkincareScreenState();
}

class _AddSkincareScreenState extends State<AddSkincareScreen> {
  ...
  late String _selectedSkinTypeKey;
  ...
  final List<String> _categoryKeys = [
    ...
    'sunscreen', 'mask', 'peeling', 'eyecream',
  ];

  final List<String> _skinTypeKeys = ['oily', 'dry', 'combo', 'normal', 'sensitive'];
  ...
  String _skinTypeKeyFromLabel(String label) {
    for (final key in _skinTypeKeys) {
      if (tr('skin_$key') == label) return key;
    }
    return _skinTypeKeys.first;
  }
  ...
  void initState() {
    ...
    _selectedCategoryKey = item != null ? _categoryKeyFromLabel(item.category) : 'cleanser';
    _selectedSkinTypeKey = item != null ? _skinTypeKeyFromLabel(item.skinType) : 'combo';
  ...
  void _saveItem() {
    if (_formKey.currentState!.validate()) {
      ...
      if (_isEditing) {
        ...
        item.skinType = tr('skin_$_selectedSkinTypeKey');
        ...
        final newItem = SkincareItem(
          id: const Uuid().v4(),
          name: _nameController.text.trim(),
          category: tr('cat_$_selectedCategoryKey'),
          brand: _brandController.text.trim(),
          skinType: tr('skin_$_selectedSkinTypeKey'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(_isEditing ? tr('edit') : tr('add_skincare_title'))),
      body: Padding(
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              ...
              const SizedBox(height: 16),
              DropdownButtonFormField<String>(
                initialValue: _selectedSkinTypeKey,
                decoration: InputDecoration(labelText: tr('form_skin_type')),
                items: _skinTypeKeys.map((key) {
                  return DropdownMenuItem(value: key, child: Text(tr('skin_$key')));
                }).toList(),
                onChanged: (value) {
                  setState(() {
                    _selectedSkinTypeKey = value!;
                  });
                },
              ),
